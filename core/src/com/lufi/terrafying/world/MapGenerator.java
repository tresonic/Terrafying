...
public class MapGenerator {
	public static void generate(Map map, int width, int height) {
		...
		for (int x = 0; x < width; x++) {
			...
			if (nextGrass-- <= 0) {
				map.setBlock(x, stoneHeight + dirtHeight + 1, Block.getBlockByName("grassplant").getId());
				nextGrass = ThreadLocalRandom.current().nextInt(1, 5);
			}
		...
		for (int x = 1; x < width; x++) {
			double xMul = (-1 / (2 * x + 0.0000001) + 1) * (-1 / (2 * (width - x) + 0.0000001) + 1);

			for (int y = 1; y < height; y++) {
				if (((n3.getNoise(x, y) + 1) / 2) * (1 / ((double) height / ((double) height * 1000) * y + 1)) // higher
																												// ->
																												// less
																												// caves
	...
	private static void genOres(Map map, int width, int height) {
		...
		SimplexNoise gravelNoise = new SimplexNoise(16, 0.4f, ThreadLocalRandom.current().nextInt());
		SimplexNoise sandNoise = new SimplexNoise(16, 0.4f, ThreadLocalRandom.current().nextInt());
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				if (map.getBlock(x, y) == Block.getBlockByName("stone").getId()) {
					...
					if (gravelNoise.getNoise(x, y) > 0.15)
						map.setBlock(x, y, Block.getBlockByName("gravel").getId());
					if (sandNoise.getNoise(x, y) > 0.15)
						map.setBlock(x, y, Block.getBlockByName("sand").getId());
				}
			}
		}
